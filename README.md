...
The Windows `hosts` file translates hostnames to IP addresses locally, bypassing DNS resolution. This repository provides:

1. **Batch Script (`.bat`)**: Quick, zero-dependency execution for standard environment setup and toggle operations.
2. **Python Script (`.py`)**: Advanced manipulation, structured backups, entry validation, and bulk operations using regular expressions.

---

## ⚠️ Important Safety & Permissions

Modifying the `hosts` file requires **Administrator privileges** on Windows. 

* **File Location**: `C:\Windows\System32\drivers\etc\hosts`
* **Antivirus Warning**: Some antivirus software locks the `hosts` file to prevent tampering. You may need to temporarily allow access or run your terminal as an Administrator.
* **Safety First**: Both scripts in this repository automatically create a timestamped backup before applying any modifications.

---

## 🚀 Quick Start

### 1. Using the Batch Script (`manage_hosts.bat`)

The Batch script is ideal for fast environment toggles or simple append operations without requiring a Python installation.

#### How to Run
1. Right-click on `manage_hosts.bat`.
2. Select **Run as Administrator**.
3. Follow the on-screen menu prompts.

#### Core Features
* Automatic Administrative privilege check (prompts for elevation if missing).
* Fast menu-driven interface (`cmd.exe`).
* Appends custom domains directly to the system `hosts` file.

---

### 2. Using the Python Script (`Microsoft_Hosts_Website.py`)

The Python script offers robust entry parsing, domain validation, duplicate prevention, and clean removal of custom entries.

#### Requirements
* Python 3.8 or higher.
* Admin rights on the command prompt or terminal executing the script.

#### Usage Examples

Run your command prompt or terminal as **Administrator**, then execute:
